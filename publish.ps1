<#
  ...
  RUN (from a terminal opened in this repo folder):
    ...
    Update + release v4.5.2:  powershell -ExecutionPolicy Bypass -File .\publish.ps1 -Release v4.5.2
    With a commit message:    powershell -ExecutionPolicy Bypass -File .\publish.ps1 -Release v4.5.2 -CommitMessage "Subject`n`nBody"

  REDO A RELEASE (the tag must not already exist on GitHub):
    gh release delete v4.5.2 --cleanup-tag --yes
    powershell -ExecutionPolicy Bypass -File .\publish.ps1 -Release v4.5.2 -CommitMessage "..."
#>
...
$WidgetName    = "draw-advanced"   # widget folder name (must match EB folder + repo subfolder)
$RepoName      = "draw-advanced-widget"
