import { React, AllWidgetProps, jsx, WidgetState, getAppStore, appActions } from 'jimu-core';
...
// geometryEngine + geometryEngineAsync: WASM-dependent in JSAPI 5.0 - CSP blocks in local dev.
// Load fire-and-forget; widget renders immediately. Methods degrade gracefully if unavailable.
import { loadArcGISJSAPIModules } from 'jimu-arcgis';
let _geometryEngine: any = null;
let _geometryEngineAsync: any = null;
let _wasmUnavailable = false;
loadArcGISJSAPIModules(['esri/geometry/geometryEngine', 'esri/geometry/geometryEngineAsync'])
	.then(([ge, gea]) => { _geometryEngine = ge; _geometryEngineAsync = gea; })
	.catch(() => { _wasmUnavailable = true; /* unavailable in local dev (WASM/CSP) */ });
...
const manualBufferGeometry = (geom: any, distM: number, sr: any): any => {
	...
	if (geom.type === 'point') return Polygon.fromJSON({ rings: [_wCircle(geom.x, geom.y, distM, isGeo)], spatialReference: sr?.toJSON ? sr.toJSON() : sr });
	if (geom.type === 'polyline') { const p: number[][][] = geom.paths || []; if (p.length > 0) return _wPathBuf(p[0], distM, sr); }
	const ext = geom.extent;
	if (ext) { const cx = (ext.xmin + ext.xmax) / 2, cy = (ext.ymin + ext.ymax) / 2; return Polygon.fromJSON({ rings: [_wCircle(cx, cy, Math.max(ext.width, ext.height) / 2 + distM, isGeo)], spatialReference: sr?.toJSON ? sr.toJSON() : sr }); }
	return null;
};
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig>, States> {
	...
	private createBufferGeometry = async (geometry: any, distance: number, unit: string): Promise<any | null> => {
		try {
			...
			// Try JSAPI geometryEngineAsync (geodesic — needs WASM, works on Portal)
			let ge = _geometryEngineAsync;
			if (!ge && !_wasmUnavailable) {
				try {
					const [loaded] = await Promise.race([
						loadArcGISJSAPIModules(['esri/geometry/geometryEngineAsync']),
						new Promise<never>((_, rej) => setTimeout(() => rej(new Error('timeout')), 500))
					]);
					_geometryEngineAsync = loaded;
					ge = loaded;
				} catch { _wasmUnavailable = true; /* WASM blocked — skip future retries */ }
			}
			...
			if (ge) {
				...
				if (!res) { console.warn('Widget: Buffer operation returned null'); return null; }
				return Array.isArray(res) ? (res[0] ?? null) : res;
			}

			// Manual planar fallback (no WASM — local dev only)
			const distM = bufferToMeters(distance, unit);
			return manualBufferGeometry(geometry, distM, view.spatialReference);
		} catch (error) {
	...
	private confirmMultiCopyMerged = async () => {
		...
		for (const f of features) {
			if (!f.graphic.geometry) continue;
			let cat = f.graphic.geometry.type;
		...
		try {
			const geomType = features[0].graphic.geometry.type === 'extent' ? 'polygon' : features[0].graphic.geometry.type;
	...
	private getMainDrawings = (): any[] => {
		if (!this.drawLayer) return [];
		return this.drawLayer.graphics.toArray().filter(g =>
			!g.attributes?.isPreviewBuffer &&
			!g.attributes?.isBuffer &&
			!g.attributes?.isBufferDrawing &&
			!g.attributes?.isMeasurementLabel &&
	...
	private combineGeometries = async (geometries: any[]): Promise<any | null> => {
		...
		try {
			...
			if (allPolygons) {
				...
				const merged = await _geometryEngineAsync?.union(polys as any[]);
				return merged;
			}
			...
			if (allPolylines) {
				const merged = await _geometryEngineAsync?.union(geometries as any[]);
				return merged;
			}
		...
		return (
			...
			>
							...
							>
								...
								<div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
									...
									<Button
										...
										disabled={this.state.multiCopySelectedFeatures.length < 2}
										style={{ fontSize: '11px', padding: '2px 10px' }}
										title="Merge all selected features into a single drawing"
				...
				{this.props.config.enableMailingLabels && (
					<div className="d-flex flex-column align-items-center mb-1" style={{ gap: '2px' }}>
						<Button
							size="sm"
							type="default"
