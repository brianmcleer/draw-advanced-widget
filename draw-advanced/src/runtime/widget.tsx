...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig>, States> {
	 ...
	 */
	private cancelSketchVMWithLabelPreservation = (graphics: __esri.Graphic[]): number => {
	...
	private _multiCopyHighlightGraphics: __esri.Graphic[] = [];
	 ...
	 */
	private addToMultiCopySelection = (candidate: { graphic: __esri.Graphic; layerTitle: string; geometryType: string }) => {
	...
	private confirmMultiCopyMerged = async () => {
		...
		try {
			...
			let mergedGeometry: __esri.Geometry | null = null;

			if (geomType === 'polygon') {
				const polygons = geometries.map(g => {
					if (g.type === 'extent') return Polygon.fromExtent(g as __esri.Extent);
					return g as __esri.Polygon;
				});
				const geometryEngine = await import('@arcgis/core/geometry/geometryEngine');
				mergedGeometry = geometryEngine.union(polygons as __esri.Polygon[]);
			} else if (geomType === 'polyline') {
				const allPaths: number[][][] = [];
				for (const g of geometries) {
					const pl = g as __esri.Polyline;
					if (pl.paths) {
				...
				for (const g of geometries) {
					if (g.type === 'point') {
						const pt = g as __esri.Point;
						allPoints.push([pt.x, pt.y]);
					} else if (g.type === 'multipoint') {
						const mp = g as __esri.Multipoint;
						if (mp.points) {
	 ...
	 */
	private getMainDrawings = (): __esri.Graphic[] => {
		if (!this.drawLayer) return [];
	 ...
	 */
	private resolveGeometryForMailingLabels = (graphic: any): __esri.Geometry | null => {
	...
	private sendToMailingLabels = async () => {
		...
		if (!geometry && this.state.selectedGraphics?.size > 0) {
			const selected: __esri.Geometry[] = [];
	 ...
	 */
	private combineGeometries = async (geometries: __esri.Geometry[]): Promise<__esri.Geometry | null> => {
		...
		try {
			...
			if (allPolygons) {
				const polys = geometries.map(g =>
					g.type === 'extent' ? Polygon.fromExtent(g as __esri.Extent) : g
				);
				const eng = await import('esri/geometry/geometryEngineAsync');
				const merged = await eng.union(polys as __esri.Polygon[]);
				return merged;
			}
			...
			if (allPolylines) {
				const eng = await import('esri/geometry/geometryEngineAsync');
				const merged = await eng.union(geometries as __esri.Polyline[]);
				return merged;
			}
	 ...
	 */
	private addMultiCopyHighlight = (geometry: __esri.Geometry) => {
		const view = this.state.currentJimuMapView?.view;
		if (!view || !geometry) return;
	 ...
	 */
	private handleSpatialSelectionComplete = async (selectionGeometry: __esri.Geometry) => {
		const view = this.state.currentJimuMapView?.view;
		if (!view) return;
		...
		try {
			if (selectedCopyLayerId) {
				...
				for (const mil of mapImageLayers) {
					const sublayers = (mil as __esri.MapImageLayer).sublayers?.toArray() || [];
	...
	private querySingleLayerWithGeometry = async (
		...
		geometry: __esri.Geometry
	): Promise<Array<{ graphic: __esri.Graphic; layerTitle: string; geometryType: string }>> => {
		const results: Array<{ graphic: __esri.Graphic; layerTitle: string; geometryType: string }> = [];
