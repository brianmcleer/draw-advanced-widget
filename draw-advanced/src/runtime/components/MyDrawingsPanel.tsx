...
`;
            ...
            this.sketchViewModel.on("update", (event) => {
                ...
                try {
                    ...
                    if ((event.state === "start" || event.state === "active") && event.graphics.length > 0) {
                        const hasLockedGraphic = event.graphics.some((gra: __esri.Graphic) =>
     ...
     */
    isDrawingLocked = (graphic: __esri.Graphic): boolean => {
        const uid = graphic?.attributes?.uniqueId;
        ...
        if (newLocked.has(uid)) {
            ...
            if (this.sketchViewModel?.state === 'active') {
                const isBeingEdited = this.sketchViewModel.updateGraphics?.some(
                    (g: __esri.Graphic) => g === graphic || g.attributes?.uniqueId === uid
                );
                if (isBeingEdited) {
                    this.sketchViewModel.cancel();
                }
        ...
        try {
            ...
            if (this.props.jimuMapView?.view?.spatialReference) {
                ...
                if (needsProjection) {
                    ...
                    // Try ArcGIS client-side projection engine.
                    // This is the ONLY engine that correctly handles ESRI-specific CRS
                    // like Krovak with X_Scale/Y_Scale/XY_Plane_Rotation parameters.
                    try {
                        if (!this.projectionLoaded) {
                            if (!this.projectionLoadPromise) {
                                this.projectionLoadPromise = projection.load().then(() => {
                    ...
                    // Fallback to proj4 ONLY for WKID-based sources (not WKT).
                    // proj4 CANNOT correctly handle ESRI-specific CRS parameters like
                    // Krovak's X_Scale/Y_Scale/XY_Plane_Rotation — it produces
                    // plausible-looking but WRONG coordinates.
                    if (sourceWkid && !sourceWkt) {
                        try {
                            const proj4Module = await import('proj4');
                            const proj4fn = (proj4Module.default || proj4Module) as any;
                            const targetEpsg = (targetSR as any)?.latestWkid || targetWkid;
                            const transform = proj4fn(`EPSG:${sourceWkid}`, `EPSG:${targetEpsg}`);
                            ...
                            if (geometry.type === 'point') {
                                ...
                                const newPaths = (geometry as __esri.Polyline).paths.map((path: number[][]) =>
                                    path.map((pt: number[]) => {
                                ...
                                const newRings = (geometry as __esri.Polygon).rings.map((ring: number[][]) =>
                                    ring.map((pt: number[]) => {
                                        const p = transform.forward(pt);
                                        return pt.length > 2 ? [p[0], p[1], pt[2]] : p;
                                    })
        ...
        try {
            ...
            if (geomType === 'polygon') {
                ...
                const polygons = geometries.map(g => {
                    if (g.type === 'extent') {
                        return Polygon.fromExtent(g as __esri.Extent);
                    }
