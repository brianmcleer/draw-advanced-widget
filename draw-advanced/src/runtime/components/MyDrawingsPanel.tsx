...
let _projection: any = null;
let _geometryEngine: any = null;
let _geometryEngineAsync: any = null;
loadArcGISJSAPIModules([
    ...
    _projection = proj;
    _geometryEngine = ge;
    _geometryEngineAsync = gea;
    proj.load().catch(() => { /* WASM unavailable in local dev */ });
...
const _mdpManualBuffer = (geom: any, distM: number, sr: any): any => {
    ...
    if (geom.type === 'point') return Polygon.fromJSON({ rings: [_mdpCircleRing(geom.x, geom.y, distM, isGeo)], spatialReference: sr?.toJSON ? sr.toJSON() : sr });
    if (geom.type === 'polyline') { const p: number[][][] = geom.paths || []; if (p.length > 0) return _mdpPathBuf(p[0], distM, sr); }
    const ext = geom.extent;
    if (ext) { const cx = (ext.xmin + ext.xmax) / 2, cy = (ext.ymin + ext.ymax) / 2; return Polygon.fromJSON({ rings: [_mdpCircleRing(cx, cy, Math.max(ext.width, ext.height) / 2 + distM, isGeo)], spatialReference: sr?.toJSON ? sr.toJSON() : sr }); }
    return null;
};
...
export class MyDrawingsPanel extends React.PureComponent<MyDrawingsPanelProps, MyDrawingsPanelState> {
    ...
    private projectGeometryToWGS84 = async (geometry: any): Promise<any | null> => {
        try {
            ...
            if (_projection) { try { await _projection.load(); } catch (_) { } }
            ...
            const projectedGeometry = _projection?.project(geometry as any, wgs84SR) as any;
    ...
    private projectGeometryToWGS84Alternative = async (geometry: any): Promise<any | null> => {
        try {
            ...
            // Try ArcGIS client-side projection engine (WASM)
            try {
                if (_projection) { try { await _projection.load(); } catch (_) { } }
                const projected = _projection?.project(geometry as any, wgs84SR) as any;
                if (projected) return projected;
            } catch (projectionError) {
                console.warn('ArcGIS PE failed in WGS84 export, trying proj4:', projectionError);
            }
    ...
    private projectGeometryFromWGS84 = async (wgs84Geometry: any, targetSR: any): Promise<any | null> => {
        try {
            ...
            // Try ArcGIS projection engine (WASM)
            try {
                if (!this.projectionLoaded) {
                    if (!this.projectionLoadPromise) {
                        this.projectionLoadPromise = _projection?.load().then(() => {
                ...
                const projectedResult = _projection?.project(wgs84Geometry as any, targetSR);

                if (projectedResult) {
                    const projectedGeometry = Array.isArray(projectedResult) ? projectedResult[0] : projectedResult;
                    if (projectedGeometry) return projectedGeometry as any;
                }
            } catch (projectionError) {
                console.warn('ArcGIS projection engine failed, trying proj4:', projectionError);
            }
...
`;
    ...
    private createBufferGeometry = async (geometry: any, distance: number, unit: string): Promise<any | null> => {
        try {
            ...
            if (!view) return null;
            if (_geometryEngineAsync) {
                try {
                    const linearUnit = unit as any;
                    const res = (view.spatialReference?.isGeographic || view.spatialReference?.isWebMercator)
                        ? await _geometryEngineAsync.geodesicBuffer(geometry as any, distance, linearUnit)
                        : await _geometryEngineAsync.buffer(geometry as any, distance, linearUnit, true);
                    if (!res) { console.warn('Buffer operation returned null'); return null; }
                    return Array.isArray(res) ? (res[0] ?? null) : res;
                } catch { /* fall through to manual buffer */ }
            }
            const result = _mdpManualBuffer(geometry, _mdpToMeters(distance, unit), view.spatialReference);
            if (!result) console.warn('Manual buffer returned null for:', geometry?.type);
            return result;
        } catch (error) {
    ...
    initializeComponents = () => {
        ...
        if (this.sketchViewModel) {
            this.sketchViewModel.watch('updateGraphics', (newGraphics: any) => {
            ...
            if (this.props.jimuMapView?.view?.spatialReference) {
                ...
                if (needsProjection) {
                    ...
                    try {
                        ...
                        // Direct projection
                        let projectedGeometry = _projection?.project(geometry, targetSR);
                        ...
                        if (sourceWkid !== 4326 && targetWkid !== 4326) {
                            const wgs84SR = new SpatialReference({ wkid: 4326 });
                            const toWGS84 = _projection?.project(geometry, wgs84SR);
                            if (toWGS84) {
                                const toTarget = _projection?.project(toWGS84, targetSR);
                                if (toTarget) return toTarget;
                            }
                        }

                        // With geographic transformation
                        const transformations = _projection?.getTransformations(
                            geometry.spatialReference, targetSR
                        );
                        if (transformations?.length > 0) {
                            projectedGeometry = _projection?.project(geometry, targetSR, transformations[0]);
                            if (projectedGeometry) {
                                return projectedGeometry;
                            }
                        }
                    } catch (peError) {
                        console.error('❌ ArcGIS PE threw error:', peError);
                    }
            ...
            try {
                ...
                if (importedGraphics.length > 0) {
                    ...
                    if (geometries.length > 0) {
                        const unionGeometry = _geometryEngine?.union(geometries);
    ...
    private performMergeSelected = async (selectedIndices: number[], selectedDrawings: ExtendedGraphic[]) => {
        ...
        try {
            ...
            if (geomType === 'polygon') {
                ...
                const geometryEngine = null /* dynamic import removed */;
                mergedGeometry = _geometryEngine?.union(polygons as any[]);
                                                    ...
                                                    {!this.state.collapsedDrawings.has(index) && (
                                                        <div className="button-container">
                                                            ...
                                                            <Button
                                                                ...
                                                                title={this.isDrawingLocked(graphic) ? 'Unlock to rename' : 'Rename'}
                                                            >
                                                                Rename
                                                            </Button>
                                                            ...
                                                            <Button
                                                                ...
                                                                title={this.isDrawingLocked(graphic) ? 'Unlock to delete' : 'Delete'}
                                                            >
                                                                Delete
                                                            </Button>
                                                        </div>
