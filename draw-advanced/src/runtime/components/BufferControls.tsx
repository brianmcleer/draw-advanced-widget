...
// geometryEngineAsync: WASM-based in JSAPI 5.0, blocked by CSP in local ExB dev.
// Loaded fire-and-forget; falls back to manual buffer math if unavailable.
let _geometryEngineAsync: any = null;
let _wasmUnavailable = false;
loadArcGISJSAPIModules(['esri/geometry/geometryEngineAsync'])
    .then(([gea]) => { _geometryEngineAsync = gea; })
    .catch(() => { _wasmUnavailable = true; /* unavailable in local dev */ });
...
const pathBuffer = (path: number[][], distM: number, sr: any): any => {
    ...
    const ring: number[][] = [
        ...left,
        ...arcPoints(path[n - 1][0], path[n - 1][1], distM, endCapFrom),
        ...[...right].reverse(),
        ...arcPoints(path[0][0], path[0][1], distM, startCapFrom),
        left[0],
    ];
...
const polygonBuffer = (rings: number[][][], distM: number, sr: any): any => {
    ...
    const offsetRing = (ring: number[][]): number[][] => {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (cosHalf < 0.15) {
                // Near-180° corner → bevel
                result.push([pts[i][0] + ea.nx * distM, pts[i][1] + ea.ny * distM]);
                result.push([pts[i][0] + eb.nx * distM, pts[i][1] + eb.ny * distM]);
            } else {
                const miterLen = Math.min(distM / cosHalf, distM * 4);
...
const manualBuffer = (geometry: any, distMeters: number, sr: any): any => {
    ...
    if (geometry.type === 'point') {
        return makePoly([circleRing(geometry.x, geometry.y, distMeters, isGeo)], sr);
    }
    if (geometry.type === 'polyline') {
        const paths: number[][][] = geometry.paths || [];
        if (paths.length > 0) return pathBuffer(paths[0], distMeters, sr);
    }
    if (geometry.type === 'polygon') {
        return polygonBuffer(geometry.rings || [], distMeters, sr);
    }
    const ext = geometry.extent;
    if (ext) {
        const cx = (ext.xmin + ext.xmax) / 2, cy = (ext.ymin + ext.ymax) / 2;
        return makePoly([circleRing(cx, cy, Math.max(ext.width, ext.height) / 2 + distMeters, isGeo)], sr);
    }
    return null;
};
...
export const BufferControls: React.FC<BufferControlsProps> = ({ jimuMapView, sketchViewModel }) => {
    ...
    const createBufferGeometry = async (geometry: any, distance: number, unit: string) => {
        try {
            const view = jimuMapView?.view;
            if (!view) return null;
            // geometryEngineAsync loaded lazily - see module-level block
            const linearUnit = unit as any;
            const ge = _geometryEngineAsync;
            const distMeters = toMeters(distance, unit);
            const sr = view.spatialReference;
            if (ge && !_wasmUnavailable) {
                const res = (sr?.isGeographic || sr?.isWebMercator)
            ...
            // Manual planar fallback (no WASM) — good enough for dev testing
            return manualBuffer(geometry, distMeters, sr);
        } catch (e) {
